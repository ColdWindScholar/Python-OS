...
def python_modules_check():
    ...
    if not sys.modules.get("tkinter"):
        module_not_found("tkinter")  # Jajaj, its impossible... but just in case..
    elif not sys.modules.get("psutil"):
        module_not_found("psutil")
    elif not sys.modules.get("pyllow"):
        module_not_found("pyllow")
    else:
...
def get_desktop_enviroment() -> str:
    ...
    if sys.platform in ["win32", "cygwin"]:
        ...
        if desktop_session is not None:  # easier to match if we doesn't have  to deal with caracter cases
            ...
            if desktop_session in ["gnome", "unity", "cinnamon", "mate", "xfce4", "lxde", "fluxbox", "blackbox",
            ...
            # There is no guarantee that they will not do the same with the other desktop environments.
                ...
                return "razor-qt"  # e.g. razorkwin
            elif desktop_session.startswith("wmaker"):
                return "windowmaker"  # e.g. wmaker-common
...
class KRNL_Bug_check:
    ...
    def __init__(self, master: Misc, error_id: str, msg_color: str, bg_color: str) -> None:
        ...
        def start():
            ...
            self.id = Label(self.base, text="Error ID: " + self.error_id, font=("Segoe UI", 11), bg=self.bg_color,
                            fg=self.msg_color_2)
            self.brief_info = Label(self.base, text="Brief info: " + self.error_info, font=("Segoe UI", 12),
                                    bg=self.bg_color, fg=self.msg_color_2)
            ...
            self.comment_label = Label(self.base, text=self.comment, font=("Segoe UI", 11), bg=self.bg_color,
                                       fg=self.msg_color_2)
